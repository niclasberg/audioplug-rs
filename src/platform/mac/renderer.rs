...
use objc2_core_graphics::{
    CGColor, CGContext, CGContextAddArcToPoint, CGContextAddLineToPoint, CGContextClipToRect,
    CGContextClosePath, CGContextConcatCTM, CGContextFillEllipseInRect, CGContextFillPath,
    CGContextFillRect, CGContextGetCTM, CGContextMoveToPoint, CGContextRestoreGState,
    CGContextSaveGState, CGContextScaleCTM, CGContextSetFillColorWithColor, CGContextSetLineWidth,
    CGContextSetStrokeColorWithColor, CGContextStrokeEllipseInRect, CGContextStrokePath,
    CGContextStrokeRectWithWidth, CGContextTranslateCTM,
};
...
impl<'a> RendererRef<'a> {
    ...
    pub fn draw_rectangle(&mut self, rect: Rectangle, color: Color, line_width: f32) {
        ...
        unsafe {
            CGContextStrokeRectWithWidth(Some(&self.context), rect.into(), line_width.into())
        };
    }
    ...
    pub fn draw_ellipse(&mut self, origin: Point, radii: Size, color: Color, line_width: f32) {
        ...
        let rect = Rectangle::new(
            origin - Vector::new(radii.width, radii.height),
            radii.scale(2.0),
        );
