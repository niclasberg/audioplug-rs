...
use x11rb::{COPY_DEPTH_FROM_PARENT, connection::Connection, protocol::xproto::{ConnectionExt, CreateWindowAux, EventMask, WindowClass}};
...
impl X11Window {
    pub fn open(app: &mut X11Application, handler: Box<dyn WindowHandler>) -> Result<Self, Error> {
        ...
        app.connection.create_window(COPY_DEPTH_FROM_PARENT, 
            id, 
            app.screen().root, 
            0, 0, 640, 480, 0, WindowClass::INPUT_OUTPUT, 0, &values).unwrap();
