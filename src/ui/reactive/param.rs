...
impl<P: AnyParameter> ParamSetter<P> {
    ...
    pub fn info<'a, 'cx>(&self, cx: &'a mut impl CanRead<'cx>) -> &'a dyn AnyParameter
    where
        'cx: 'a,
    {
        cx.read_context()
    ...
    pub fn begin_edit<'cx>(&self, cx: &mut impl CanWrite<'cx>) {
        cx.write_context().host_handle().begin_edit(self.id);
    }

    pub fn set_value_normalized<'cx>(&self, cx: &mut impl CanWrite<'cx>, value: NormalizedValue) {
    ...
    pub fn set_value_plain<'cx>(&self, cx: &mut impl CanWrite<'cx>, value: PlainValue) {
        let mut write_context = cx.write_context();
        let param_ref = write_context.get_parameter_ref(self.id);
    ...
    pub fn end_edit<'cx>(&self, cx: &mut impl CanWrite<'cx>) {
        cx.write_context().host_handle().end_edit(self.id);
    }
}

impl<P: Parameter> ParamSetter<P> {
    pub fn set_value<'cx>(&self, cx: &mut impl CanWrite<'cx>, value: P::Value) {
        let write_context = cx.write_context();
        let param_ref = write_context.get_parameter_ref(self.id);
